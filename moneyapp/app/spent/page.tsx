...
export default function SpendingPage() {
  ...
  function save(newTransactions: Transaction[], newBudget = budget) {
    ...
    const updated: UserData = { ...fullData, transactions: newTransactions, budget: newBudget };
    setFullData(updated);
  ...
  function addExpense() {
    ...
    const newTx: Transaction = {
      id: Date.now(), type: "subtract", amount: value,
    ...
    <div className="min-h-screen w-full pb-28"
      ...
      <div className="px-4 max-w-7xl mx-auto flex flex-col gap-4">
        ...
        <motion.div initial="hidden" animate="show" variants={fadeUp}
          ...
          {/* On iPad: big number left, progress right */}
          <div className="flex flex-col md:flex-row md:items-center md:gap-8">

            {/* Left: spent amount + TTS */}
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex justify-center md:justify-start mt-3">
                <button onClick={() => speak(`You have spent ${spent.toFixed(2)} euro this month`)}
            ...
            {budget > 0 && (
              <div className="flex-1 mt-5 md:mt-0">
                <div className="flex justify-between text-xs font-semibold mb-2">
                  ...
                  <span style={{ color: percent >= 90 ? "#dc2626" : "#ea580c" }}>{Math.round(percent)}% used</span>
                </div>
                <div className="w-full h-4 bg-orange-50 rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${percent}%` }}
                    ...
                    style={{ background: percent >= 90 ? "#dc2626" : "linear-gradient(90deg, #fb923c, #ea580c)" }} />
                </div>
                {/* Budget chips */}
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div className="rounded-2xl p-3 text-center" style={{ background: "#fff7ed" }}>
                    ...
                    <p className="text-xl font-extrabold text-orange-600">€{spent.toLocaleString()}</p>
                  </div>
                  <div className="rounded-2xl p-3 text-center" style={{ background: "#fff7ed" }}>
                    <p className="text-[11px] text-orange-500 font-semibold">Remaining</p>
                    <p className="text-xl font-extrabold text-orange-600">€{Math.max(budget - spent, 0).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <motion.div initial="hidden" animate="show" variants={fadeIn}
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
              {[100, 200, 300, 500, 750, 1000].map((b) => (
                <button key={b} onClick={() => { setBudget(b); save(transactions, b); }}
          ...
          <motion.div initial="hidden" animate="show" variants={fadeIn}
            ...
            {/* Preset amounts — 4 on phone, 8 on iPad */}
            <div className="grid grid-cols-4 gap-2 mb-3">
              {[5, 10, 20, 50].map((v) => (
                <button key={v} onClick={() => { setPresetAmount(String(v)); setAmount(String(v)); }}
            ...
            {/* Custom amount */}
            <input type="number" placeholder="Or type amount" value={amount}
              onChange={(e) => { setAmount(e.target.value); setPresetAmount(""); }}
            ...
            <button onClick={addExpense}
              ...
              style={{ background: selectedCat.color, boxShadow: `0 4px 20px ${selectedCat.color}55` }}>
        ...
        <motion.div initial="hidden" animate="show" variants={fadeIn}>
          <button onClick={resetSpending}
            className="w-full py-4 rounded-2xl font-semibold text-sm transition-all active:scale-95"
            style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.7)", border: "1.5px solid rgba(255,255,255,0.2)" }}>
